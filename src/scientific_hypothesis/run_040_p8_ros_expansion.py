"""run_040: P8 ROS family expansion — exploit vs design principle test.

Tests whether the ROS single-point dependence (run_039) can be distributed across
an expanded oxidative stress bridge family. 6 conditions build up progressively:

  C_P7_FULL          — All 10 P7 metabolites (run_038 control)
  C_ROS_CORE         — ROS + Glutathione only
  C_ROS_ENZYMES      — ROS-core + SOD + Catalase
  C_ROS_REGULATORS   — ROS-core + HO-1 + NRF2
  C_ROS_ALL          — All 7 ROS family nodes
  C_ROS_ALL_NO_CORE  — 5 new nodes only (no ROS+GSH) ← KEY: can this replace core?

Pre-registration: runs/run_040_p8_ros_expansion/preregistration.md
...
RUN_DIR = os.path.join(BASE_DIR, "runs", "run_040_p8_ros_expansion")

# Load caches from run_038 and run_039 (all P7 paths already covered)
R38_EVIDENCE = os.path.join(BASE_DIR, "runs", "run_038_p7_kg_expansion", "evidence_cache.json")
R39_PUBMED = os.path.join(BASE_DIR, "runs", "run_039_ablation", "pubmed_cache.json")

EVIDENCE_CACHE_PATH = os.path.join(RUN_DIR, "evidence_cache.json")
PUBMED_CACHE_PATH = os.path.join(RUN_DIR, "pubmed_cache.json")

ENTITY_TERMS: dict[str, str] = {**BASE_ENTITY_TERMS, **P7_ENTITY_TERMS, **P8_ENTITY_TERMS}

# ---------------------------------------------------------------------------
# Condition definitions — each specifies which bridge metabolites to KEEP.
# All other bridge nodes are removed from the P8 KG.
# ---------------------------------------------------------------------------

# ROS-core nodes (the P7 pair that run_039 identified as critical)
ROS_CORE: list[str] = [
    "chem:metabolite:reactive_oxygen_species",
    "chem:metabolite:glutathione",
]

# P8 new nodes (the 5 expanded oxidative stress members)
P8_NEW_NODES: list[str] = [
...
CONDITION_KEEP: dict[str, list[str]] = {
    ...
    "C_ROS_CORE": ROS_CORE,
    "C_ROS_ENZYMES": ROS_CORE + [
        "chem:metabolite:superoxide_dismutase",
        "chem:metabolite:catalase",
    ],
    "C_ROS_REGULATORS": ROS_CORE + [
        "chem:metabolite:heme_oxygenase_1",
        "chem:metabolite:nrf2",
    ],
    "C_ROS_ALL": ROS_CORE + P8_NEW_NODES,
    "C_ROS_ALL_NO_CORE": P8_NEW_NODES,  # KEY TEST: no ROS+GSH
}
...
def ablate_kg_keep(p8_kg: dict[str, Any], keep_ids: list[str]) -> dict[str, Any]:
    """Return a KG with only the specified bridge metabolites active.

    Removes all other chemistry-domain metabolite bridge nodes and their edges.
    Non-bridge nodes (base KG biology + chemistry nodes) are always kept.
    """
    keep_set = set(keep_ids)
    remove_set = set(ALL_BRIDGE_IDS) - keep_set
    nodes = [n for n in p8_kg["nodes"] if n["id"] not in remove_set]
    edges = [
        e for e in p8_kg["edges"]
...
def bucketed_top_k(
    ...
    """Bucketed R2 selection (T3 design, same quotas as run_038)."""
...
# ---------------------------------------------------------------------------
# Attribution dispersion analysis
# ---------------------------------------------------------------------------

def dispersion_analysis(results: dict[str, dict]) -> dict[str, Any]:
    """Measure how distributed the breakthrough is within the expanded ROS family.

    Primary test: does C_ROS_ALL_NO_CORE beat run_039's C_NO_ROS baseline (0.9286)?
    Secondary: per-node contribution within C_ROS_ALL.
    """
    run039_c_no_ros_inv = 0.9286  # from run_039 — the baseline to beat

    ros_all = results.get("C_ROS_ALL", {})
    no_core = results.get("C_ROS_ALL_NO_CORE", {})
    ros_core = results.get("C_ROS_CORE", {})
    p7_full = results.get("C_P7_FULL", {})

    no_core_inv = no_core.get("T3_investigability_rate", 0.0)
    ros_all_inv = ros_all.get("T3_investigability_rate", 0.0)

    # Key test: has single-point dependence been reduced?
    fragility_improved = no_core_inv > run039_c_no_ros_inv

    # Determine P8 verdict
    if no_core_inv > 0.943:
        p8_verdict = "DESIGN_PRINCIPLE"
    elif no_core_inv > 0.929:
        p8_verdict = "PARTIAL_DISTRIBUTION"
    elif no_core_inv > run039_c_no_ros_inv:
        p8_verdict = "FRAGILE_IMPROVEMENT"
    else:
        p8_verdict = "STILL_CONCENTRATED"

    # Build-up gradient
    build_up = {
        c: results.get(c, {}).get("T3_investigability_rate", 0.0)
        for c in CONDITIONS
    }

    # Contribution of enzymes and regulators (relative to ROS-core)
    ros_enzymes_inv = results.get("C_ROS_ENZYMES", {}).get("T3_investigability_rate", 0.0)
    ros_regulators_inv = results.get("C_ROS_REGULATORS", {}).get("T3_investigability_rate", 0.0)

    enzyme_contribution = round(ros_enzymes_inv - ros_core.get("T3_investigability_rate", 0.0), 4)
    regulator_contribution = round(ros_regulators_inv - ros_core.get("T3_investigability_rate", 0.0), 4)

    # H_P8_1: C_ROS_ALL achieves STRONG_SUCCESS?
    h_p8_1 = ros_all.get("T3_outcome") == "STRONG_SUCCESS"
    # H_P8_2: C_ROS_ALL_NO_CORE beats run_039 C_NO_ROS?
    h_p8_2 = fragility_improved
    # H_P8_3: monotonic build-up?
    inv_sequence = [build_up.get(c, 0) for c in [
        "C_ROS_CORE", "C_ROS_ENZYMES", "C_ROS_REGULATORS", "C_ROS_ALL"]]
    h_p8_3 = all(inv_sequence[i] <= inv_sequence[i + 1] + 0.01
                 for i in range(len(inv_sequence) - 1))
    # H_P8_4: C_P7_FULL still STRONG_SUCCESS?
    h_p8_4 = p7_full.get("T3_outcome") == "STRONG_SUCCESS"

    return {
        "p8_verdict": p8_verdict,
        "run039_c_no_ros_inv": run039_c_no_ros_inv,
        "c_ros_all_no_core_inv": no_core_inv,
        "fragility_improved": fragility_improved,
        "improvement_vs_run039": round(no_core_inv - run039_c_no_ros_inv, 4),
        "build_up_investigability": build_up,
        "enzyme_contribution_vs_core": enzyme_contribution,
        "regulator_contribution_vs_core": regulator_contribution,
        "h_p8_1_ros_all_strong_success": h_p8_1,
        "h_p8_2_fragility_reduced": h_p8_2,
        "h_p8_3_monotonic_buildup": h_p8_3,
        "h_p8_4_p7_still_strong": h_p8_4,
    }
...
def main() -> None:
    """Run P8 ROS expansion experiment (run_040)."""
    os.makedirs(RUN_DIR, exist_ok=True)
    ts = datetime.now().isoformat()

    # Build P8 KG
    print("=" * 60)
    print("Building P8 KG (base → P7 → P8 extended ROS family)")
    p8_kg = build_p8_from_base()
    m = p8_kg["metadata"]
    print(f"  P8 KG: {m['node_count']} nodes, {m['edge_count']} edges")
    print(f"  New nodes: {m['p8_new_nodes']} | New edges: "
          f"{m['p8_bio_to_chem_edges'] + m['p8_chem_to_bio_edges']}")

    # Load caches (run_038 evidence + run_039 pubmed)
    evidence_cache = _load_cache(R38_EVIDENCE)
    pubmed_cache = _load_cache(R39_PUBMED)
    print(f"  Evidence cache: {len(evidence_cache)} entries (run_038)")
    print(f"  PubMed cache: {len(pubmed_cache)} entries (run_039)")
    ...
    disp = dispersion_analysis(results)
    print(f"  P8 Verdict: {disp['p8_verdict']}")
    print(f"  C_ROS_ALL_NO_CORE inv: {disp['c_ros_all_no_core_inv']:.4f} "
          f"(vs run_039 C_NO_ROS: {disp['run039_c_no_ros_inv']:.4f})")
    print(f"  Improvement vs run_039: {disp['improvement_vs_run039']:+.4f}")
    print(f"  Fragility improved: {disp['fragility_improved']}")
    print(f"  H_P8_1 (ROS_ALL STRONG_SUCCESS): {disp['h_p8_1_ros_all_strong_success']}")
    print(f"  H_P8_2 (fragility reduced): {disp['h_p8_2_fragility_reduced']}")
    print(f"  H_P8_3 (monotonic build-up): {disp['h_p8_3_monotonic_buildup']}")
    print(f"  H_P8_4 (P7_FULL still STRONG): {disp['h_p8_4_p7_still_strong']}")
    ...
    _save_cache(disp, os.path.join(RUN_DIR, "dispersion_analysis.json"))
    _save_run_config(ts, disp, results)
...
def _save_run_config(ts: str, disp: dict, results: dict) -> None:
    """Save run_config.json."""
    cfg = {
        "run_id": "run_040_p8_ros_expansion",
        "timestamp": ts,
        "phase": "P8",
        "seed": SEED,
        "top_k": TOP_K,
        ...
        "p8_verdict": disp.get("p8_verdict", ""),
        "h_p8_1": disp.get("h_p8_1_ros_all_strong_success", False),
        "h_p8_2": disp.get("h_p8_2_fragility_reduced", False),
        "h_p8_3": disp.get("h_p8_3_monotonic_buildup", False),
        "h_p8_4": disp.get("h_p8_4_p7_still_strong", False),
        "outcomes": {
