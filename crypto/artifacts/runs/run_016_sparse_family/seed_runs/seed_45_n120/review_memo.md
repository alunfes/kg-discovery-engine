...
- **beta_reversion**: count=4, mean=0.7085, median=0.7056, p90=0.7663, count_norm_top_k=1.0005, ev_slope=0.0207, arch_advantage=False
- **other**: count=16, mean=0.6249, median=0.6124, p90=0.707, count_norm_top_k=1.0001, ev_slope=-0.0048, arch_advantage=False
- **flow_continuation**: count=10, mean=0.6235, median=0.6118, p90=0.6461, count_norm_top_k=1.0002, ev_slope=0.0044, arch_advantage=False

## F2: Cross-Branch Normalization

- mean_abs_rank_diff: 11.23
- max_rank_diff: 28
...
- **alt_led**: n=35, dominant=positioning_unwind, mean_score=0.6719, top_k_share=1.0
- **btc_led**: n=25, dominant=positioning_unwind, mean_score=0.6656, top_k_share=1.0
- **flat_oi**: n=50, dominant=positioning_unwind, mean_score=0.6576, top_k_share=1.0
- **funding_quiet**: n=4, dominant=beta_reversion, mean_score=0.7085, top_k_share=1.0
...
- **high_vol**: n=10, dominant=flow_continuation, mean_score=0.6235, top_k_share=1.0
- **low_vol**: n=4, dominant=beta_reversion, mean_score=0.7085, top_k_share=1.0
...
- mean_uplift_by_branch: {'positioning_unwind': 0.0314, 'beta_reversion': 0.0347, 'other': -0.1061, 'flow_continuation': -0.0901}
- top_uplift (complexity-adjusted):
  ...
  - [beta_reversion] E1 beta reversion: (ETH,BTC) — no funding shift, n uplift=0.0904 conf=low
  - [beta_reversion] E1 beta reversion: (ETH,BTC) — transient aggressio uplift=0.0840 conf=low
...
- n_top_k_changed: 0
- mean_uplift_aware_score: 0.4445
- Top-5 uplift-aware cards:
  ...
  5. [beta_reversion] E1 beta reversion: (ETH,BTC) — no funding shift, n ua=0.8012 raw=0.774 rank_Δ=-1
...
- actionable_watch top cards:
  ...
  - [beta_reversion] E1 beta reversion: (ETH,BTC) — transient aggression, no pers score=0.748
- reject_conflicted examples:
- monitor_borderline cases:
  - [beta_reversion] E1 beta reversion: (ETH,BTC) — transient aggression, no soft_gated=False
...
- High-urgency (actionable_watch) cards:
  ...
  - [beta_reversion] E1 beta reversion: (ETH,BTC) — transient aggression, no label=beta_reversion_watch
...
> Correlation break (HYPE,ETH) rho=0.095. OneSidedOIBuildNode (score=1.000, duration=60) → PositionCrowdingStateNode. Crowd unwind expected.
...
> Correlation break (HYPE,SOL) rho=0.064, break_score=0.437. FundingPressureRegimeNode (score=1.000) → FragilePremiumStateNode → UnwindTriggerNode (type=funding_extreme). Positioning unwind expected within 1-2 epochs.
...
> Correlation break (HYPE,ETH) rho=0.095. FragilePremiumStateNode (score=0.800) → PositioningUnwindContextNode. Premium dislocation → expected funding → compression cascade expected.
...
> Correlation break (ETH,BTC) rho=0.000, break_score=0.000. KG shows NoFundingShiftNode + NoOIExpansionNode (neg_evidence=0.939) → beta recoupling expected within 2-4 epochs.
...
> Correlation break between HYPE and ETH (rho=0.095, break_score=0.379) is linked via KG path to an extreme long funding rate on HYPE (z=0.00).  Positioning unwind is the likely resolution mechanism. A full 3-hop B3 chain (aggression → premium → expected_funding → realized_funding) was also found, confirming the causation.
